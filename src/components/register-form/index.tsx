...
export const RegisterForm = ({teams}:{teams: Team[]}) => {
    ...
    const formSchema = z.object({
        ...
        document: z.string().transform(onlyNumber).refine((val) => val.length >= 11, { message: t("pages.register.error.document") }),
        team: z.number(),
        info: z.string().optional(),
        phoneNumber : z.string().transform(onlyNumber).refine((val) => val.length >= 11, { message: t("pages.register.error.phoneNumber") }),  
        birthday: z.date(),
        gender: z.enum(["m", "f", "o", ""]),
    })
